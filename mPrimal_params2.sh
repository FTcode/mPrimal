...
mkdir -p ./data/mPrimal240/c5 ./logs
export LD_LIBRARY_PATH=$HOME/QD-install/lib:$LD_LIBRARY_PATH

module load python
source g6k/activate
python3 -u exp_successprob.py c=5 r=29 m=30 sigma=12 q=30000 betaQ=${betaQ} structured=${structured} \
    > ./data/mPrimal240/c5/betaQ${betaQ}_struct${structured}_trial${trial}.txt \
    2>&1