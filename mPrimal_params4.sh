...
mkdir -p ./data/mPrimal240/c7 ./logs
export LD_LIBRARY_PATH=$HOME/QD-install/lib:$LD_LIBRARY_PATH

module load python
source g6k/activate
python3 -u exp_successprob.py c=7 r=18 m=21 sigma=10 q=10000 betaQ=${betaQ} structured=${structured} \
    > ./data/mPrimal240/c7/betaQ${betaQ}_struct${structured}_trial${trial}.txt \
    2>&1