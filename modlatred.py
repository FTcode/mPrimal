...
from numpy import array, block, identity, zeros
...
def random_qary(m, n, q):
    ...
    """    
    assert(m >= n)
    # This is the (transpose) of the systematic form for a q-ary kernel lattice of A in ZZ^{n x m}
...
class ModuleLatticeReduction(Siever):
    ...
    def bkz(self, beta, tours=1, verbose=False):
        ...
        :param verbose: 
        """
        #TODO: Add a "clean" flag which will halt the process if nothing was changed on previous tour
    ...
    def profile_Q(self, rescale=False):
        ...
        WARNING: Because of the scaling in the cyclic embedding, the volume (sum of profile) is not correct;
        too large by a term d*log(c)*K_rank/2
        """        
        self.M.update_gso()
